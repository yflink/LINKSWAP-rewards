...
const hydration = {
  ...
  positions: async ({ state, hydrate }) => {
    Object.values(state?.pool || {}).forEach((pool) => hydrate('pool.userPosition', pool.address));
  },
  ...
  pool: {
    ...
    details: async ({ state, update, hydrate }, poolAddress) => {
      ...
      // token addresses
      fields.token0.address = await fields.contracts.pool.methods.token0().call();
      fields.token1.address = await fields.contracts.pool.methods.token1().call();

      // token contracts
      fields.token0.contract = new web3.eth.Contract(ERC20, fields.token0.address);
      fields.token1.contract = new web3.eth.Contract(ERC20, fields.token1.address);

      // token symbols
      fields.token0.symbol = await fields.token0.contract.methods.symbol().call();
      fields.token1.symbol = await fields.token1.contract.methods.symbol().call();
    ...
    values: async ({ state, update, hydrate }, poolAddress) => {
      ...
      const token0Contract = new web3.eth.Contract(ERC20, fields.token0.address);
      const token1Contract = new web3.eth.Contract(ERC20, fields.token1.address);

      // hydrate token balances
      fields.token0.balance = await token0Contract.methods.balanceOf(address).call();
      fields.token1.balance = await token1Contract.methods.balanceOf(address).call();

      update(`pool.${poolAddress}`, fields, () => hydrate('pool.userPosition', poolAddress));
    },
    ...
    userPosition: async ({ state, update }, poolAddress) => {
      ...
      try {
        if (reward.ert) {
          const ert_rewards = await StakingRewardsContract.methods.earned(accountAddress, 1).call();
  ...
  account: {
    ...
    fetchEthBalance: ({ state, set }) => {
      web3.eth
        ...
        .then((wei) => set('account.balance.eth', parseFloat(web3.utils.fromWei(wei)).toFixed(2)));
    },
    fetchTokenBalance: async ({ state, set }, address) => {
      ...
      contract.methods
        ...
        .call()
        .then((wei) => {
          set(`account.balance.${address}`, parseFloat(web3.utils.fromWei(wei)).toFixed(2));
        })
  ...
  'account.status': (status, { state, trigger, hydrate }) => {
    switch (status) {
      case AcountStatus.CONNECTED:
        ...
        trigger('account.fetchTokenBalance', state.network?.params?.tokenAddress);
        hydrate('positions');
        break;
