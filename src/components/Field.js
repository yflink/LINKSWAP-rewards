...
const FieldTitleText = styled(({ children, className, ...rest }) => (
  <span className={`-title-text ${className}`} {...rest}>
    {children}
  </span>
))`
  display: block;
...
const FieldInfo = styled(({ children, className, ...rest }) => (
  <span className={`-info-text ${className}`} {...rest}>
    {children}
  </span>
))`
  display: block;
...
const FieldTemplate = styled(
  ({ title, info, errors = [], footer, type, children, className, ...rest }) => (
    <span className={`field -${type} ${className}`} data-has-error={errors.length > 0} {...rest}>
      {(title || info) && (
      ...
      {errors.length > 0 && (
        <span className="-error" dangerouslySetInnerHTML={{ __html: errors[0] }} />
      )}
    </span>
  ),
)`
...
const Input = ({ value = '', onChange = () => {}, children, ...rest }) => {
  useEffect(() => {
...
const URL = ({ value = '', onChange = () => {}, children, ...rest }) => {
  useEffect(() => {
    value && onChange(value);
  }, []); // eslint-disable-line
...
const Number = styled(({ value, onChange = () => {}, min, max, children, ...rest }) => {
  useEffect(() => {
    !!value && onChange(value);
  }, []); // eslint-disable-line
...
const Custom = styled(({ children, ...rest }) => (
  <FieldTemplate {...pick(rest, ['title', 'info', 'footer', 'right', 'className'])} type="custom">
    {children}
  </FieldTemplate>
))`
  .-container {
...
const TextArea = styled(
  ({ value = '', required = false, validation = {}, onChange = () => {}, className, ...rest }) => {
    ...
    return (
      <span className={`field -input ${className}`} data-has-error={errors.length > 0 && touched}>
        <textarea value={value || ''} onChange={(e) => validate(e.target.value)} {...rest} />
        {errors.length > 0 && touched && <span className="error">{errors[0]}</span>}
      </span>
    );
  },
)`
...
const Select = styled(({ value = '', options = [], onChange = () => {}, ...rest }) => (
  <FieldTemplate
    {...pick(rest, ['title', 'info', 'errors', 'footer', 'large', 'className'])}
    type="select"
  >
...
const Toggle = styled(({ value = false, onChange = () => {}, on, off, ...rest }) => {
  ...
  return (
    <FieldTemplate
      {...pick(rest, ['title', 'info', 'errors', 'footer', 'large', 'className', 'inline'])}
      type="toggle"
    >
      <span
        className="toggle"
  ...
  .toggle {
    ...
    .-text {
      ...
      .-off {
        ...
        ${({ on, off }) => {
          const length =
            on?.text?.length && off?.text?.length
              ? on?.text?.length > off?.text?.length
                ? on?.text.length
                : off?.text.length
              : 1;
...
const Group = styled(({ children, before, after, className, ...rest }) => (
  <div className={`field-group ${className}`} {...rest}>
...
Group.Child = styled(({ children, className, ...rest }) => (
  <span className={`field-group-child ${className}`} {...rest}>
    {children}
  </span>
))`
  display: flex;
