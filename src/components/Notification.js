...
import { ReactComponent as IconError } from '@icon/check.svg';
import { ReactComponent as IconRefresh } from '@icon/spinner.svg';
import { ReactComponent as IconMinus } from '@icon/check.svg';
...
const Notification = styled(({ type, title, text, links = [], className }) => (
  <div className={`notification -item ${className}`} data-type={type}>
    <span className="-left">{Icons[type]}</span>
      ...
      {text && (
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: text.length > 100 ? `${text.substring(0, 97)}...` : text,
          }}
        />
      )}
      {links.length > 0 && (
...
const Container = styled(({ className }) => (
  <ToastContainer
    autoClose={false}
    closeButton={null}
