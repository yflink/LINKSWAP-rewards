...
const storeController = (
  ...
  stores[name] = {
    ...
    initialState: JSONCycle(initialState), // swapped
    preInitState: {},
    state: initialState,
    hydration: hydration,
    triggers: triggers,
    ...
    init: () => {
      ...
      // init with state
      let initState = init(stores[name].buildCallbackObject());
      ...
      Object.keys(subscribers).forEach((key) =>
        stores[name].subscriptions.subscribe(key, subscribers[key]),
      );
      ...
      // fire onUpdate CB
      stores[name].subscriptions._triggerCallback(
        stores[name].onUpdate,
    ...
    hydrate: async (action, payload, callback = () => {}) => {
      const func = get(stores[name].hydration, action);
      if (typeof func === 'function') {
        await func(stores[name].buildCallbackObject(), payload);
    ...
    trigger: (action, payload, callback = () => {}) =>
      new Promise(async (resolve) => {
        const func = get(stores[name].triggers, action);

        if (typeof func === 'function') {
          const result = await func(stores[name].buildCallbackObject(), payload);

          if (typeof callback === 'function') {
            callback(stores[name].buildCallbackObject());
          }

          resolve(stores[name].buildCallbackObject(), result);
        }
      }),
    _state: {
      ...
      update: async (key, payload = {}, callback) => {
        let state = get(stores[name].state, key);

        if (typeof get(stores[name].state, key) === 'object' && typeof payload === 'object') {
          state = set(
            stores[name].state,
            key,
            Object.assign(get(stores[name].state, key), payload),
          );
        } else if (!get(stores[name].state, key)) {
          state = set(stores[name].state, key, payload);
        }

        stores[name]._state._update(state, key, payload, callback);
      },
      unset: (key, callback) => {
      ...
      _update: (newState, action, payload, callback = () => {}) => {
        ...
        //const oldStateHash = stores[name].stateHash
        ...
        //if(newStateHash !== oldStateHash){  // REMOVED DEPENDENCY 17 NOV 2020 | issue with heavily nested callbacks
        ...
        if (
          payload &&
          stores[name]?.persist === true &&
          stores[name]?.persistent.includes(action)
        ) {
    ...
    subscriptions: {
      ...
      fire: (key, payload) => {
        ...
        if (keyParts.length >= 3) {
          ...
          // remove first item
          keyParts.splice(0, 1);
          const parentPayload = get(stores[name].state, keyParts.join('.'));
      ...
      _triggerCallback: (callback, payload) => {
        typeof callback === 'function' && callback(payload, stores[name].buildCallbackObject());
      },
    },
...
export const FetchStore = (name, config) => {
  name = name || 'store';
  return !stores[name] ? storeController(name, config) : stores[name];
};

export default (name, config = {}) => {
  const store = FetchStore(name, config);
  ...
  return {
    ...
    hydrate: async (action, payload, cb) => await store.hydrate(action, payload, cb),
